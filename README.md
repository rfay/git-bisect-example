...
Although Drupal 11.3.x is supposed to work with PHP 8.1+, it's not working here with PHP 8.3.30 We can see that if we `ddev launch /core/install.php` we'll get an error.

Use `git bisect` to find out when PHP 8.3.30 stopped working.

1. Check out the repo, `git clone https://github.com/rfay/git-bisect-example`
2. `ddev config --php-version=8.3` (Gets current default DDEV PHP, 8.3.30 as of 2/2026)
